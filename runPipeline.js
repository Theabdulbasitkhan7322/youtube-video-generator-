...
// Finds the most recently modified file in ./output matching a pattern
...
async function runPipeline(topic, format = 'SHORT') {
  ...
  const scriptFile = await findLatestFile(/^script_\d+\.txt$/);
  ...
  const scenesFile = await findLatestFile(/_scenes\.json$/);
  ...
  const fullScenesFile = await findLatestFile(/_fullScenes\.json$/);
  ...
  const audioFile = await findLatestFile(/^voice_\d+\.mp3$/);
  const timestampFile = await findLatestFile(/_sentences\.json$/);
  console.log(`  ✓ Audio: output/${audioFile}`);
  console.log(`  ✓ Timestamps: output/${timestampFile}`);

  // ── STAGE 4: Build Remotion project ───────────────────
  // Patch buildRemotionProject.js config with this run's actual filenames
